  ...
  switch (session.state) {
    ...
    case "AWAITING_ZONE": {
      ...
      if (isYes) {
        ...
        if (customer) {
          // Returning customer — go straight to pack, greet by name
          await updateSession(phone, { state: "AWAITING_PACK" });
          await sendWelcome(phone);
        } else {
          // New customer — get name first
          await updateSession(phone, { state: "AWAITING_NAME" });
          await sendMessage(phone, `¡Brutal! Bienvenido a Chef Papi 👨‍🍳\n\n¿Cuál es tu nombre completo?`);
        }
      } else if (isNo) {
...
async function handleNCFSelection(phone, session, input) {
  if (input === "ncf_b02") {
    ...
    // Re-fetch session with updated order then charge
    const customer = await getCustomer(phone);
    const address = session.pending_order?.delivery_address || customer?.delivery_address;
    await createOrderAndCharge(phone, { ...session, pending_order: updatedOrder }, address);
...
async function handleRNCInput(phone, session, input) {
  ...
  const updatedOrder = { ...session.pending_order, rnc: input };
  await updateSession(phone, { pending_order: updatedOrder });
  const customer = await getCustomer(phone);
  const address = session.pending_order?.delivery_address || customer?.delivery_address;
  await createOrderAndCharge(phone, { ...session, pending_order: updatedOrder }, address);
...
async function continueToCustomerInfo(phone, session) {
  const customer = await getCustomer(phone);
  if (customer) {
    // Returning customer - confirm address before payment
    await updateSession(phone, { state: "AWAITING_ADDRESS" });
    await sendAddressConfirmButtons(phone, customer.delivery_address);
  } else {
    // Shouldn't happen but fallback
    const address = session.pending_order?.delivery_address;
    await createOrderAndCharge(phone, session, address);
  }
}
...
async function handleAddressInput(phone, session, input) {
  if (input === "same_address") {
    const customer = await getCustomer(phone);
    // Keep existing address, go to pack
    await updateSession(phone, {
      state: "AWAITING_PACK",
      pending_order: { ...session.pending_order, delivery_address: customer.delivery_address },
    });
    await sendWelcome(phone);
    return;
  }
  ...
  // Save customer and go to pack
  const name = session.pending_order?.customer_name;
  if (name) {
    await upsertCustomer(phone, { name, delivery_address: input });
    await updateSession(phone, {
      state: "AWAITING_PACK",
      pending_order: { customer_name: name, delivery_address: input },
    });
  } else {
    // Returning customer updating address
    const customer = await getCustomer(phone);
    if (customer) await upsertCustomer(phone, { name: customer.name, delivery_address: input });
    await updateSession(phone, {
      state: "AWAITING_PACK",
      pending_order: { delivery_address: input },
    });
  }
  await sendWelcome(phone);
}
...
async function sendWelcome(phone) {
  ...
  await sendWA(phone, {
    ...
    interactive: {
      ...
      action: {
        buttons: [
          { type: "reply", reply: { id: "3",  title: "Pack 3 — RD$900" } },
          { type: "reply", reply: { id: "5",  title: "Pack 5 — RD$1,500" } },
          { type: "reply", reply: { id: "10", title: "Pack 10 — RD$3,000" } },
        ],
      },
    },
  });
...
// START
// ============================================================
app.listen(PORT, () => console.log(`🍗 Chef Papi v14 running on port ${PORT}`));
