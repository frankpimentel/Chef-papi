// ============================================================
// CHEF PAPI — WhatsApp Ordering Backend v3
...
async function handleMessage(phone, input) {
  ...
  // New customer — always show welcome buttons
  if (!session) {
  ...
  switch (session.state) {
    ...
    case "AWAITING_QTY":
      if (isQuestion(input) && isNaN(parseInt(input))) {
        const answer = await askAI(input);
        await sendMessage(phone, answer);
        await sendMessage(phone, "Dicho eso, ¿cuántos quieres? Escribe el número:");
      } else {
...
async function handleFlavorSelection(phone, session, input) {
  ...
  await sendMessage(phone,
    `${flavor.emoji} *${flavor.title}* — ¿cuántos quieres? (te quedan *${slotsLeft} slots*)\n\nEscribe el número:`
  );
}
...
async function handleQtyInput(phone, session, input) {
  ...
  if (qty > slotsLeft) {
    await sendMessage(phone,
      `Solo te quedan *${slotsLeft} slots*. ¿Cuántos ${flavor.emoji} ${flavor.title} quieres? (máximo ${slotsLeft})`
    );
    return;
  }
...
function isQuestion(input) {
  ...
  const commands = ["3","5","10","confirm","change","reorder","new_order","same_address","new_address","ncf_b01","ncf_b02","human"];
...
// START
// ============================================================
app.listen(PORT, () => console.log(`🍗 Chef Papi v5 running on port ${PORT}`));
