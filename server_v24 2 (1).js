...
function getDeliveryMessage(zone) {
  ...
  if (zone === "distrito") {
    if (isBeforeCutoff()) {
      return {
        message: "Estás en el Distrito Nacional. Tu orden llegará hoy en 2-3 horas, antes de las 6:30PM.",
        estimated: "Hoy 2-3 horas",
        zone: "distrito"
      };
    } else {
      return {
        message: "Como es después de las 3:30PM, tu entrega llegará mañana por la mañana. Así nos aseguramos que llegue en perfectas condiciones.",
  ...
  switch (session.state) {
    ...
    case "AWAITING_ZONE": {
      ...
      if (isYes) {
        ...
        // All three go to pack — zone validated at address entry
        ...
        if (input === "district_no") {
          await sendMessage(phone, "Tu entrega llegará en las próximas 24 horas. Ingresa tu dirección al finalizar y te confirmamos el tiempo exacto.");
        } else if (input === "district_unsure") {
          await sendMessage(phone, "No hay problema. Ingresa tu dirección al finalizar y nosotros validamos. Si estás dentro del Distrito Nacional, tu orden llega en 2-3 horas. Si estás fuera, en las próximas 24 horas.");
        }
...
async function createOrderAndCharge(phone, session, address) {
  ...
  const baseUrl = process.env.CARDNET_LIVE === "true"
    ? `https://pay.cardnet.com.do/checkout?order=${newOrder.id}&amount=${order.price + 120}`
    : `${process.env.TEST_PAYMENT_URL || "https://chef-papi-test.carrd.co"}?order=${newOrder.id}&amount=${order.price + 120}&server=https://chef-papi-production.up.railway.app`;
...
async function notifyMBE(order, orderNum) {
  try {
    ...
    const summary = order.order_items.map(i => `${FLAVORS[i.flavor]?.emoji} ${FLAVORS[i.flavor]?.title}`).join(", ");
    const now = getSDTime();
    const timeStr = now.toLocaleString("es-DO", { timeZone: "America/Santo_Domingo" });
...
// START
// ============================================================
app.listen(PORT, () => console.log(`🍗 Chef Papi v24 running on port ${PORT}`));
