  ...
  switch (session.state) {
    ...
    case "AWAITING_ZONE": {
      ...
      if (isYes) {
        ...
        if (customer) {
          // Returning customer — confirm address first
          await updateSession(phone, { state: "AWAITING_ADDRESS", pending_order: {} });
          await sendAddressConfirmButtons(phone, customer.delivery_address);
        } else {
          // New customer — get name first
          await updateSession(phone, { state: "AWAITING_NAME", pending_order: {} });
...
async function continueToCustomerInfo(phone, session) {
  // Address already collected at start — go straight to payment
  const customer = await getCustomer(phone);
  const address = session.pending_order?.delivery_address || customer?.delivery_address;
...
async function handleAddressInput(phone, session, input) {
  ...
  if (name) {
    // New customer - save name + address
...
// START
// ============================================================
app.listen(PORT, () => console.log(`🍗 Chef Papi v15 running on port ${PORT}`));
