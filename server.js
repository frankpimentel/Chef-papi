...
async function createOrderAndCharge(phone, session, address) {
  ...
  if (items.length === 0) {
    console.error("⚠️ WARNING: No items to insert for order", newOrder.id, "| selections:", order.selections, "| full pending_order:", JSON.stringify(order));
  } else {
    console.log("✅ Inserting items for order", newOrder.id, ":", JSON.stringify(items));
  }
...
app.post("/payment-confirm", async (req, res) => {
  ...
  try {
    ...
    console.log("order_items for", orderNum, ":", JSON.stringify(order.order_items));
...
// START
// ============================================================
app.listen(PORT, () => console.log(`🍗 Chef Papi v37 running on port ${PORT}`));
