...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    // Deduplicate — Meta sometimes sends the same webhook twice
...
async function handleAddressInput(phone, session, input) {
  if (input === "same_address") {
    ...
    const updatedOrder = {
      ...
      delivery_zone: zone !== "unknown" ? zone : "Por confirmar",
  ...
  await updateSession(phone, {
    ...
    pending_order: { 
      ...
      selections: session.pending_order?.selections || [],
      pack_size: session.pending_order?.pack_size || null,
      price: session.pending_order?.price || null,
    },
  });
  await sendWelcome(phone);
}
...
async function createOrderAndCharge(phone, session, address) {
  ...
  if (!order.delivery_zone && address) {
    ...
    const delivery = getDeliveryMessage(zone);
    order.delivery_zone = zone !== "unknown" ? zone : "Por confirmar";
  ...
  if (items.length === 0) {
    console.error("⚠️ WARNING: No items to insert for order", newOrder.id, "| selections:", order.selections, "| full order:", JSON.stringify(order));
  }
...
// START
// ============================================================
app.listen(PORT, () => console.log(`🍗 Chef Papi v29 running on port ${PORT}`));
